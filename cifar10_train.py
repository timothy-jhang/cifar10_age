...
tf.app.flags.DEFINE_string('train_dir', './cifar10_train_single',
...
#    print('>>>>> input resized = ',images)
    ...
    with tf.train.MonitoredTrainingSession(
      ...
      num_iter = 0.0; true_cnt = 0.0; true_cnt10=0.0
      while not mon_sess.should_stop():
        _, predictions = mon_sess.run([train_op, top_k_op])
        this_cnt = np.sum(predictions)
        true_cnt += this_cnt
        true_cnt10 += this_cnt
        num_iter += FLAGS.batch_size
        if num_iter % (FLAGS.log_frequency*FLAGS.batch_size) == 0:
          print('>>> batch accuracy = %.3f' % (true_cnt10/(FLAGS.log_frequency*FLAGS.batch_size)),  'total accuracy= %.3f' % (1.0* true_cnt/num_iter) )
          true_cnt10 = 0
